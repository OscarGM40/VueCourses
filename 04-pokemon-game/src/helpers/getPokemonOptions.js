...
const getPokemonOptions = async () => {
  ...
  // Usa slice para remover elementos sin mutar el arreglo original.Creará una copia y opera sobre esa.
  //En este caso cortamos el original con splice,asi no creamos otro
...
export const capitalize = (data) => {
  return data.toUpperCase().charAt(0) + data.slice(1);
};
...
export const getPokemonNames = async ([a, b, c, d] = []) => {
  ...
  return [
    {
      name: capitalize(pk1.data.name),
      id: pk1.data.id,
    },
    {
      name: capitalize(pk2.data.name),
      id: pk2.data.id,
    },
    {
      name: capitalize(pk3.data.name),
      id: pk3.data.id,
    },
    {
      name: capitalize(pk4.data.name),
      id: pk4.data.id,
    },
  ];
};
